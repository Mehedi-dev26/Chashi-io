...
type Props = {
  /** Show the "নতুন সাব-নোড" add button + dialog (default true) */
  showAddButton?: boolean;
  /** Show the SM / Water-Level summary cards below the grid (default true) */
  showSummary?: boolean;
  /** Show the "কীভাবে কাজ করে?" help card (default true) */
  showHelp?: boolean;
  /** Optional header title / subtitle shown above the grid */
  header?: { title: string; subtitle?: string } | null;
};

export function SubNodesNetwork({ showAddButton = true, showSummary = true, showHelp = true, header = null }: Props) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    const ch = supabase.channel("subnodes_live")
  ...
  const add = async () => {
    ...
    const payload = {
      user_id: user.id, device_id: form.device_id, label: form.label,
      notes: form.notes || null, zone_id: form.zone_id || null,
    };
  ...
  return (
    <div className="space-y-5">
      {(header || showAddButton) && (
        <div className="flex items-start justify-between flex-wrap gap-3">
          {header && (
            <div className="flex items-center gap-2.5">
              <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-emerald-500 to-sky-500 grid place-items-center shadow-lg shrink-0">
                <Sprout className="h-5 w-5 text-white" />
              </div>
              <div>
                <h2 className="text-base font-extrabold">{header.title}</h2>
                {header.subtitle && <p className="text-xs text-muted-foreground">{header.subtitle}</p>}
                <p className="text-[11px] text-muted-foreground mt-0.5">
                  {bn(onlineCount)}/{bn(nodes.length)} অনলাইন · রিয়েল-টাইম sub-node নিয়ন্ত্রণ
                </p>
              </div>
            </div>
          )}
      ...
      {showHelp && (
        <Card>
          <CardHeader><CardTitle className="text-base flex items-center gap-2"><Cpu className="h-4 w-4" />কীভাবে কাজ করে?</CardTitle></CardHeader>
          <CardContent className="text-sm text-muted-foreground space-y-2">
            <p>• প্রতিটি sub-node (ESP8266) প্রতি ৫ সেকেন্ডে heartbeat পাঠায় → আপনি এখানে লাইভ ডেটা দেখেন।</p>
            <p>• <b className="text-foreground">SM sensor</b> (YL-69 + YL-38 comparator) থেকে raw analog পড়া → calibration mapping → মাটির আর্দ্রতা শতাংশে (SM%) রূপান্তর।</p>
            <p>• <b className="text-foreground">পানির স্তর</b> SM থেকে derived — শুকনা মাটিতে কম, সিক্ত মাটিতে বেশি।</p>
            <p>• <b className="text-foreground">Servo motor (SG90)</b> দিয়ে পানির লাইন on/off।</p>
            <p>• <b className="text-foreground">Online check</b>: heartbeat ১৫ সেকেন্ডের বেশি না এলে নোড offline দেখানো হয়।</p>
          </CardContent>
        </Card>
      )}
            ...
            return (
              <div className="space-y-4">
                <div className="rounded-lg border border-indigo-500/20 bg-indigo-500/5 p-3 text-xs leading-relaxed">
                  ...
                  <p className="text-muted-foreground">নিচের মানগুলো হুবহু কপি করে আপনার Arduino IDE-তে sub-node sketch-এর উপরে paste করুন।</p>
                </div>
                ...
                {!zoneId && (
                  <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 p-3 text-xs text-amber-800 dark:text-amber-200">
                    ⚠ এই নোডটি এখনো কোনো জমিতে assign করা হয়নি।
                  </div>
                )}
                <div>
